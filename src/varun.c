...
static unsigned int rn;		//delta
static unsigned int rd;
static unsigned int rm;
static int masking = 0xff000000;
...
void decode()
{
  ...
  if ((instruction_word>>27)&1 == 1)                        //branch instr
  {
  	...
  	if ((instruction_word>>25)&1 == 0)
  	{
	  	  if ((instruction_word>>28) == 0) 
	  	  {
...
//-------------------------------------------------------------------------------------------------------
  //DELTA LOTS OF GADBAD + change print
  ...
  else														//data processing
  {
  	...
  	if((instruction_word>>25)&1 == 0)						//not immediate
	{
		rm = (instruction_word)&15;
		operand2 = R[rm];
